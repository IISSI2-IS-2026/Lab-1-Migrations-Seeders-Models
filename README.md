...
* Alternatively you can run and debug your project by using the _Run and Debug_ tool of VSCode. It can be found on the left-sided bar or by typing `shift+ctrl+D`, and selecting `Debug Backend` in the drop down list. Add a breakpoint at the first line of the funcion `initializeServer` in app.js, and click on the play icon in the _Run and Debug_ tool to debug this file. Inspect the `server` variable.
...
In order to make a minimal test, we have included the following code at `src/controllers/RestaurantController.js` and `src/routes/RestaurantRoutes.js` (we will address the details of implementing routes and controllers in the next session lab):
